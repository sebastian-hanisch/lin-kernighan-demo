...
def _run(lk_step=1, **state):
    at = AppTest.from_file(APP, default_timeout=300)
    for k, v in state.items():
        at.session_state[k] = v
    at.run()
    if lk_step != 1:
        at.select_slider(key="lk_step").set_value(lk_step).run()
    return at
...
def test_default_run_has_no_exception_and_shows_the_measured_default():
    at = _run()
    _ok(at)
    assert _metric(at, "Chained LK: beste Tour") == "0.1 %" and _metric(at, "Chained LK, Tiefe 1") == "0.1 %" and _metric(at, "Ein einzelner Abstieg") == "4.8 %"
    assert any("Gleichauf" in s.value for s in at.info) or any("zahlt sich aus" in s.value for s in at.success) or any("besser" in s.value for s in at.warning)
...
def test_every_preset_button_runs(name):
    ...
    _ok(at)
    p = C.PRESETS[name]
    assert at.session_state["depth_slider"] == p["max_depth"] and at.session_state["budget_select"] == p["budget"] and at.session_state["n_slider"] == p["n"]
    assert at.metric
...
def test_every_step_runs(step, n):
    at = _run(n_slider=n, budget_select=25000, lk_step=step)
    _ok(at)
    assert at.get("plotly_chart") and at.session_state["lk_step"] == step


def test_step_three_iteration_slider_and_play_kicks():
    at = _run(lk_step=3, budget_select=25000)
    _ok(at)
    lv = next(s for s in at.slider if s.key == "lk_iter")
    assert lv.value == lv.max
    lv.set_value(0).run()
    _ok(at)
    at2 = _run(lk_step=3, budget_select=25000)
...
def test_depth_extremes_run():
    for depth in (C.DEPTH_MIN, C.DEPTH_MAX):
        _ok(_run(depth_slider=depth, budget_select=25000))
...
@pytest.mark.parametrize("kw", [dict(n_slider=200, budget_select=50000), dict(n_slider=10, ballung_slider=100, budget_select=10000),
                                 dict(depth_slider=1, budget_select=25000), dict(depth_slider=8, budget_select=25000)])
...
def test_permalink_values_are_clamped_and_snapped():
    ...
    at.query_params["depth"] = "9999"
    at.query_params["budget"] = "12345"
    at.run()
    _ok(at)
    assert at.session_state["n_slider"] == C.N_MAX and at.session_state["ballung_slider"] == 50
    assert at.session_state["depth_slider"] == C.DEPTH_MAX and at.session_state["budget_select"] == C.DEFAULT_BUDGET


def test_sweeps_run_on_demand():
    at = _run(n_slider=10, budget_select=10000)
    at.selectbox(key="sweep_select").set_value("max_depth").run()
...
def test_experiments_run_on_demand(monkeypatch):
    ...
    monkeypatch.setitem(ev.SWEEP_VALUES, "max_depth", (1, 2))
    ...
    at = _run(n_slider=10, budget_select=10000)
    for key, flag in (("restart_start", "restart_on"), ("budget_start", "budget_on"), ("spread_start", "spread_on"), ("scaling_start", "scaling_on")):
...
def test_footer_and_grenzen_are_present():
    ...
    assert any("Die tiefere Suche wird gezielt wiederverwendet" in m.value for m in at.markdown)
